...
from datetime import datetime, timedelta
import hashlib
import sqlite3  # Direct SQLite for fallback
...
# ========== DATABASE SETUP ==========
# Use direct SQLite connection as fallback if SQLAlchemy fails
app.config["SQLALCHEMY_DATABASE_URI"] = f"sqlite:///{DATABASE_PATH}"
app.config["SQLALCHEMY_BINDS"] = {
    'admin': f"sqlite:///{ADMIN_DB_PATH}"
}
...
try:
    ...
    print(f"⚠️ SQLAlchemy init warning: {e}")
    db = None
...
# ========== DIRECT SQLITE FUNCTIONS (FALLBACK) ==========
def init_sqlite_db():
...
def initialize_admin_user():
    ...
    session['full_name'] = 'System Administrator'
    session['role'] = 'super_admin'
    print("✅ Using simple session-based authentication")
...
def health_check():
    ...
    return jsonify({
        ...
        "database": "connected" if os.path.exists(DATABASE_PATH) else "not_connected"
    })

# =======================================================
#   API: SIMPLIFIED GET STUDENTS
# =======================================================
@app.route("/api/get-students", methods=["GET"])
@login_required
def get_students():
...
# =======================================================
#   API: SIMPLIFIED MARK ATTENDANCE
# =======================================================
@app.route("/api/mark-attendance", methods=["POST"])
@login_required
def mark_attendance():
    ...
    try:
        ...
        # For demo, accept manual entry or face recognition
        if student_id or enrollment_number:
            conn = sqlite3.connect(DATABASE_PATH)
            cursor = conn.cursor()
            ...
            if student:
                ...
                cursor.execute('''
                    INSERT INTO attendance (student_id, subject, date, time, confidence, status)
                    VALUES (?, ?, ?, ?, ?, ?)
                ''', (student[0], subject, date, time, 0.95, "Present"))
                ...
                return jsonify({
                    "success": True,
                    "recognized": True,
                    "message": f"Attendance marked for {student[1]}",
                    "attendance_record": {
                        "student_id": student[0],
                        "student_name": student[1],
                        "enrollment_number": student[2],
                        "subject": subject,
                        "date": date,
                        "time": time,
                        "confidence": 0.95,
...
def get_attendance():
    ...
    try:
        ...
        query = '''
            SELECT a.*, s.name, s.enrollment_number 
            FROM attendance a
        ...
        if date_filter:
            query += ' WHERE a.date = ?'
            cursor.execute(query + ' ORDER BY a.marked_at DESC LIMIT ?', (date_filter, limit))
        else:
            cursor.execute(query + ' ORDER BY a.marked_at DESC LIMIT ?', (limit,))
...
def initialize_system():
    ...
    print(f"   • Base Directory: {BASE_DIR}")
    print(f"   • Server Directory: {SERVER_DIR}")
    print(f"   • Database: {'✅ Connected' if os.path.exists(DATABASE_PATH) else '⚠️ Not found'}")
    print(f"   • Total Students in DB: {student_count}")
    print(f"   • Total Encodings in Model: {len(model_data['encodings'])}")
    print(f"   • Model File: {'✅ Exists' if os.path.exists(MODEL_PATH) else '⚠️ Not found'}")
    print(f"   • Authentication: ✅ Ready (admin/admin123)")
...
if __name__ == "__main__":
    ...
    print(f"🌐 Access at: http://{host}:{port}/sign-in")
